...
$handler->run(function(JobHandler $handler) use ($overwrite) {
	$patchUpdater = new PatchUpdater();

	switch ($handler->job->action) {
		case UpdateJobAction::DOWNLOAD_CHAMPION_IMAGES:
			$patchUpdater->downloadChampionImgs($handler->patchContext->patchNumber, $overwrite, job: $handler->job);
			break;
		case UpdateJobAction::DOWNLOAD_ITEM_IMAGES:
			$patchUpdater->downloadItemImgs($handler->patchContext->patchNumber, $overwrite, job: $handler->job);
			break;
		case UpdateJobAction::DOWNLOAD_SPELL_IMAGES:
			$patchUpdater->downloadSummonerImgs($handler->patchContext->patchNumber, $overwrite, job: $handler->job);
			break;
		case UpdateJobAction::DOWNLOAD_RUNE_IMAGES:
			$patchUpdater->downloadRuneImgs($handler->patchContext->patchNumber, $overwrite, job: $handler->job);
			break;
		default:
