...
foreach ($teams as $tindex=>$team) {
	$team_id = $team['OPL_ID'];
	$players_from_team = $dbcn->execute_query("SELECT * FROM players JOIN players_in_teams pit on players.OPL_ID = pit.OPL_ID_player WHERE OPL_ID_team = ? AND (PUUID IS NULL OR summonerID IS NULL)", [$team_id])->fetch_all(MYSQLI_ASSOC);
