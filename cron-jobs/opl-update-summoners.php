...
ini_set("error_log", "cron_logs/cron_errors/opl_summoners_$day.log");
...
echo "\n---- getting Summonernames from OPL \n";
$results = [];

if ($group_id != NULL) {
	file_put_contents("cron_logs/cron_log_$day.log","\n----- Summonernames starting -----\n".date("d.m.y H:i:s")." : Summonernames for group $group_id\n", FILE_APPEND);
	...
	file_put_contents("cron_logs/cron_log_$day.log","\n----- Summonernames starting -----\n".date("d.m.y H:i:s")." : Summonernames for tournament $group_id\n", FILE_APPEND);
	...
	foreach ($leagues as $league) {
		file_put_contents("cron_logs/cron_log_$day.log",date("d.m.y H:i:s")." : Summonernames for league {$league["number"]} ({$league["OPL_ID"]})\n", FILE_APPEND);
		...
		foreach ($groups as $group) {
			file_put_contents("cron_logs/cron_log_$day.log",date("d.m.y H:i:s")." : Summonernames for group {$group["number"]} ({$group["OPL_ID"]})\n", FILE_APPEND);
			$players = $dbcn->execute_query("SELECT *
													FROM players
													    JOIN players_in_teams_in_tournament pit
													        ON players.OPL_ID = pit.OPL_ID_player
